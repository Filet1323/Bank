        ...
        <div class="search-bar" id="the-search-bar">
            <div class="flex-item-search-bar" id="fi-search-bar">
                ...
                <div class="flex-item-by">
                    <label id="sort">Sortuj wg. :</label>
                </div>

                <div class="flex-item-search-by">
                    <select name="by" onChange="window.location.href=this.value">
                        <option selected disabled hidden>
                            <?php if (empty($_GET['sort'])) {?>ID &darr;<?php } else { ?>
                                <?php if ($sort == 'tid_down') {?>ID &darr;<?php } ?>
                                <?php if ($sort == 'tid_up') {?>ID &uarr;<?php } ?>
                                <?php if ($sort == 'date_down') {?>Data &darr;<?php } ?>
                                <?php if ($sort == 'date_up') {?>Data &uarr;<?php } ?>
                            <?php } ?>
                        </option>
                        <option value="customer_transactions.php?sort=tid_down">ID &darr;</option>
                        <option value="customer_transactions.php?sort=tid_up">ID &uarr;</option>
                        <option value="customer_transactions.php?sort=date_down">Data &darr;</option>
                        <option value="customer_transactions.php?sort=date_up">Data &uarr;</option>
                    </select>
                </div>
    ...
    <div id="id01" class="modal">
      ...
      <form class="modal-content animate" action="" method="post">
        ...
        <div class="container">
            ...
            <p id="filter">(Wyjdź żeby usunąć filtry)</p>
          <label>Uwagi :</label>
          <input type="text" placeholder="Wpisz uwagę" name="search_term">

          <label>Trwanie (yyyy-mm-dd) :</label>
          ...
          <button id="submit" type="submit">Wybierz</button>
        </div>
